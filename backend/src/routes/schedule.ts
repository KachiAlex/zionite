...
router.get('/', async (_req, res) => {
  try {
    ...
    const schedule = await db.all(
      `SELECT id, title, day_of_week, time, type
       FROM schedule 
...
router.get('/all', authenticateToken, requireRole('admin'), async (_req, res) => {
  try {
    const db = await getDb()
    const schedule = await db.all(
      'SELECT * FROM schedule ORDER BY day_of_week, time'
    )
    res.json({ schedule })
  } catch {
    res.status(500).json({ error: 'Failed to fetch schedule' })
...
router.post('/', authenticateToken, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const id = uuidv4()
    await db.run(
      `INSERT INTO schedule (id, title, day_of_week, time, type) 
       VALUES ($1, $2, $3, $4, $5)`,
      [id, title, day_of_week, time, type]
    )
...
router.put('/:id', authenticateToken, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    await db.run(
      `UPDATE schedule SET ${updates.join(', ')} WHERE id = $${paramIndex}`,
      values
    )
