...
router.get('/radio/current', async (_req, res) => {
    try {
       ...
       ORDER BY rs.start_time DESC LIMIT 1`, [now.toISOString()]);
        ...
        const items = await db.all(`SELECT pi.id as item_id, pi.content_type, pi.content_id, pi.order_index, pi.duration_minutes,
       ...
       ORDER BY pi.order_index ASC`, [schedule.playlist_id]);
